...
This guide is for writers reviewing and editing Aphelion lore content. It assumes you already have
GitHub Desktop installed and signed in, and a local checkout of `Meridian-Rift` somewhere on disk (it
does not need to be beside this repository, but the launcher will offer that location by default).

## First run

1. Open this repository (`aphelion-lore-tools`) in GitHub Desktop and make sure it's up to date.
2. Double-click [`Launch Lore Tools.cmd`](../Launch%20Lore%20Tools.cmd).
3. If a compatible 64-bit Python 3.11+ with Pillow isn't already on your machine, the launcher asks
   ...
   `%LOCALAPPDATA%\AphelionLoreTools\runtime`. This does not touch any system-wide Python install.
...
4. The first time, the launcher asks for your `Meridian-Rift` checkout path (or offers a nearby one
   ...
   `%LOCALAPPDATA%\AphelionLoreTools\settings.json` and re-launching.
5. Your browser opens automatically to the local editor. Closing the launcher's console window stops
   the local server.
...
- The **Tools** panel runs catalog refresh, validation, and DM generation without leaving the page.
  Refresh the catalog after a game-repository update that adds or removes targets.

## Saving your work locally

The **Local Git workflow** panel in the sidebar covers this repository (`Lore tools`) and, if
...
Overrides only reach `Meridian-Rift` through the explicit staged export flow:
...
4. After a successful apply, GitHub Desktop opens automatically for the game checkout so you can
   ...
   use the **Meridian-Rift** repository panel or open GitHub Desktop manually instead.
...
repository changed since your last catalog refresh — run **Refresh catalog** in the Tools panel and
reload the page.
