...
async function loadEditorData() {
  const [catalog, icons, groups, files, tools] = await Promise.all([
  ...
  await Promise.all([loadReviewEntries(), loadRepositoryStatus(), loadExportStages()]);
}
...
function showServerLaunchMessage() {
  setText('#status-message', 'This editor must be launched through the repository server. Run: python tools/lore_editor/serve.py --repo-root . --port 0');
